...
class CtrlDisciplina():       
    ...
    def buscarDisciplinas(self):
        codigo =  simpledialog.askstring("Buscar Disciplina", "Digite o codigo", parent=self.ctrlPrincipal.root)
        disc = self.getDisciplina(codigo)
        if disc is None:
            self.limiteLista = LimiteMostraDisciplinas('Disciplina não encontrada')
        else:
            str = 'Código -- Nome\n'
            str += disc.getCodigo() + ' -- ' + disc.getNome()
            self.limiteLista = LimiteMostraDisciplinas(str)
