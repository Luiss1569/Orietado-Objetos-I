...
class CtrlEstudante():       
    ...
    def buscarEstudantes(self):
        matricula =  simpledialog.askstring("Buscar Estudante", "Digite a Matricula", parent=self.ctrlPrincipal.root)
        estudante = self.getEstudante(matricula)
        if estudante is not None:
            str = 'Nro Matric. -- Nome\n'
            str += estudante.getNroMatric() + ' -- ' + estudante.getNome() + '\n'
            self.limiteLista = LimiteMostraEstudantes(str)
        else :
            self.limiteLista = LimiteMostraEstudantes('Estudante não encontrado')