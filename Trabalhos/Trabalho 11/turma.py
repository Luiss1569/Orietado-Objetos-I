...
class CtrlTurma():       
    ...
    def getTurmas(self, codigo):
        ...
        for turma in self.listaTurmas:
            turmaDisc = turma.getDisciplina()
            if turmaDisc.getCodigo() == codigo:
                turmas.append(turma)
    ...
    def buscarTurmas(self):
        codigo =  simpledialog.askstring("Buscar Turmas", "Informe o código da disciplina", parent=self.ctrlPrincipal.root)
        turmas = self.getTurmas(codigo)
        
        if turmas == None:
            self.limiteLista = LimiteMostraTurmas('Nenhuma turma encontrada')
        else: 
            str = ''
            for turma in turmas:
                str += 'Código: ' + turma.getCodigo() + '\n'
